...
void drawRPMBarBlocks(int rpm, int maxRPM = 6000)
{
  ...
  // --- Frame mengikuti lekukan filledBlocks ---
  if (filledBlocks > 0)
  {
    // Atas: dari kiri ke kanan
    for (int i = 0; i < filledBlocks - 1; i++)
    {
      int x1 = startX + i * (blockWidth + spacing);
      int y1 = startY[i];
      int x2 = startX + (i + 1) * (blockWidth + spacing);
      int y2 = startY[i + 1];
      display.drawLine(x1, y1, x2, y2, TFT_WHITE); // garis atas antar blok
    }
    // Kiri: sisi kiri blok pertama
    int xLeft = startX;
    int yTop = startY[0];
    int yBot = startY[0] + blockHeight;
    display.drawLine(xLeft, yTop, xLeft, yBot, TFT_WHITE);

    // Bawah: dari kiri ke kanan
    for (int i = 0; i < filledBlocks - 1; i++)
    {
      int x1 = startX + i * (blockWidth + spacing);
      int y1 = startY[i] + blockHeight;
      int x2 = startX + (i + 1) * (blockWidth + spacing);
      int y2 = startY[i + 1] + blockHeight;
      display.drawLine(x1, y1, x2, y2, TFT_WHITE); // garis bawah antar blok
    }
    // Kanan: sisi kanan blok terakhir terisi
    int xRight = startX + (filledBlocks - 1) * (blockWidth + spacing) + blockWidth - 1;
    int yTopR = startY[filledBlocks - 1];
    int yBotR = startY[filledBlocks - 1] + blockHeight;
    display.drawLine(xRight, yTopR, xRight, yBotR, TFT_WHITE);
  }
  ...
  {
    int blockIdx = i * 5;
    int x = startX + blockIdx * (blockWidth + spacing) + blockWidth / 2;
    int y = startY[blockIdx] + blockHeight + 8;
    char label[2];
    sprintf(label, "%d", i);
